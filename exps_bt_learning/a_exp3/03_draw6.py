...
cfg.task_name='aaa_demo0_draw6'
# cfg.task_name='task1'
cfg.scene_file_name='scene_file_0'
cfg.target_object_name = 'electric_refrigerator.n.01_1'
...
def open(object_name,query):
    ...
    target_pos_word = th.tensor([ -1.56037, -1.09196,  1.02708])
    simulator.set_target_visual_pose(target_pos_word,size=0.02)
    simulator.idle_step(10)
    # simulator.navigate_to_pos(object_name=cfg.target_object_name,pos=target_pos_word,offset=(0.9,-0.2))
    
    # simulator.eef_reach_pos(target_pos_word,horizontal=False,obj_face_tensor = th.tensor([1.,0,0.]),grounding=True)
    # simulator.eef_reach_pos(target_pos_word,horizontal=True,obj_face_tensor = th.tensor([1.,0,0.]),grounding=True)
    
    
    
    tps = simulator.keypoint_proposal("To close the microwave,\
        point out the door of the microwave. Must point out.")
    
    
    obj_face_tensor = [1,0,0]
    yaw = math.atan2(-obj_face_tensor[1],obj_face_tensor[0])

    target_pos = target_pos_word #target_pos+obj_face_tensor*0.1
    ...
    simulator.move_hand_forward(0.3,obj_face_tensor = th.tensor([1.,0,0.]))
    
    simulator.close_gripper()
    
    simulator.move_hand_backward(0.3,obj_face_tensor = th.tensor([1.,0,0.]))
...
if __name__ == "__main__":
    ...
    # # # 打开抽屉
    cfg.target_object_name = 'microwave.n.02_1'
