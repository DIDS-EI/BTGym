...
def main():
    ...
    # 设置目标位置（在物体上方）
    target_pos = th.tensor([[0.6, 0, 0.6]], device=current_pos.device)  # 确保使用相同的device
    # 设置目标朝向（从上方抓取）
    # 从上往下
    ...
    curobo_mg = CuRoboMotionGenerator(
        robot,
        robot_cfg_path="/home/admin01/workspace/OmniGibson/omnigibson/data/assets/models/fetch/fetch_description_curobo.yaml"
    )
