...
import carb
import numpy as np
from btgym.utils.helper import add_extensions, add_robot_to_scene
...
def main():
    list_of_robots = ['fetch_description_curobo.yaml']  # [:2]
    # list_of_robots = ['franka.yml']  # [:2]
    # list_of_robots = ['franka_panda_description_curobo.yaml']  # [:2]
    usd_help = UsdHelper()
    ...
    for i in range(len(list_of_robots)):
        ...
        robot_cfg["kinematics"]["asset_root_path"] = os.path.join(og_root_path, 'data/assets',robot_cfg["kinematics"]["asset_root_path"])  
        robot_cfg_list.append(robot_cfg)

        # content_path = curobo.types.file_path.ContentPath(
        #     robot_config_absolute_path=robot_cfg_path, robot_usd_absolute_path=robot_usd_path
        # )
        # robot_cfg = curobo.cuda_robot_model.util.load_robot_yaml(content_path)["robot_cfg"]
