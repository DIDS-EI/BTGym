...
class Env:
    ...
    def close_gripper(self):
        ...
        # 设置夹爪关闭的力参数
        gripper_force = 10.0  # 夹爪力大小
        max_steps = 100  # 最大尝试步数
        
        # 获取夹爪关节的索引
        l_gripper_idx = self.robot.get_joint_idx("l_gripper_finger_joint")
        r_gripper_idx = self.robot.get_joint_idx("r_gripper_finger_joint")
        ...
        for _ in range(max_steps):
            ...
            # 创建力控制张量
            efforts = th.zeros(self.robot.n_joints)
            efforts[l_gripper_idx] = -gripper_force
            efforts[r_gripper_idx] = -gripper_force
            ...
            # 获取当前夹爪位置，检查是否完全关闭
            current_width = self.robot.get_joint_positions()[-1]
            if current_width <= 0.001:  # 设置一个小的阈值
                break
