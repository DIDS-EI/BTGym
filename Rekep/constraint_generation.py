...
class ConstraintGenerator:
    def __init__(self, config):
        self.config = config
        self.client = OpenAI(api_key=os.environ['OPENAI_API_KEY'], base_url="https://api.dwyu.top/v1") #"https://api.chatanywhere.tech/v1"
    ...
    def generate(self, img, instruction, metadata):
        ...
        # stream back the response 流式返回响应
        # 使用 OpenAI 的 API 创建一个聊天完成请求，并设置模型、提示、温度、最大令牌数和流式响应。
        ...
        #     model=self.config['model'], # gpt-4o
        #     messages=messages,
        #     temperature=self.config['temperature'], #0
        #     max_tokens=self.config['max_tokens']*5 #2048
        # )
        ...
        # save raw output 保存原始输出
        print("========= OpenAI output: ==============",output)
        with open(os.path.join(self.task_dir, 'output_raw.txt'), 'w') as f:
            f.write(output)
        # parse and save constraints 解析并保存约束
