...
from btgym.algos.bt_planning.examples import *
from btgym.algos.bt_planning.Action import state_transition
from btgym.algos.bt_planning.ReactivePlanning import ReactivePlanning
from btgym.algos.bt_planning.HOBTEA import HOBTEA
from btgym.algos.bt_planning.BTExpansion import BTExpansion
from btgym.algos.bt_planning.OBTEA import OBTEA
from btgym.algos.bt_planning.HBTP import HBTP

# Used for experimental test data recording
from btgym.algos.bt_planning_expand_exp.BTExpansion import BTExpansion as BTExpansion_test
...
class BTExpInterface:
    def __init__(self, behavior_lib, cur_cond_set, priority_act_ls=[], key_predicates=[], key_objects=[],
                 ...
                 heuristic_choice=-1, output_just_best=True, exp_record=False,continue_expand=False, max_expanded_num=None,
                 theory_priority_act_ls=None,info_dict = {}):
        """
        ...
        self.continue_expand = continue_expand
        ...
        if not self.exp_record:
            if self.selected_algorithm == "hobtea":
                self.algo = HOBTEA(verbose=False, act_tree_verbose=self.act_tree_verbose, \
                                   ...
                                   continue_expand=self.continue_expand,
                ...
                self.algo = BTExpansion(verbose=False, act_tree_verbose=self.act_tree_verbose,
                                        priority_act_ls=self.priority_act_ls, time_limit=self.time_limit,
                                        continue_expand=self.continue_expand,
                                        output_just_best=self.output_just_best,max_expanded_num=self.max_expanded_num)
                ...
                self.algo = HBTP(verbose=False, act_tree_verbose=self.act_tree_verbose,
                                        priority_act_ls=self.priority_act_ls, time_limit=self.time_limit,
                                 continue_expand=self.continue_expand,
                                        output_just_best=self.output_just_best,info_dict=self.info_dict)
            else:
            ...
            if self.selected_algorithm == "hobtea":
                self.algo = HOBTEA_test(verbose=False, act_tree_verbose=self.act_tree_verbose,
                                        ...
                                        continue_expand=self.continue_expand,max_expanded_num=self.max_expanded_num,
                ...
                self.algo = HBTP(verbose=False, act_tree_verbose=self.act_tree_verbose,
                             priority_act_ls=self.priority_act_ls, time_limit=self.time_limit,
                             output_just_best=self.output_just_best,exp_record=self.exp_record,
                                 continue_expand=self.continue_expand,max_expanded_num=self.max_expanded_num,
                                                    theory_priority_act_ls=self.theory_priority_act_ls,info_dict=self.info_dict)
            else:
                print("Error in algorithm selection: This algorithm does not exist.")
    ...
    def execute_bt(self, goal, state, verbose=True):
        # print("Start OK")
        from btgym.algos.bt_planning.tools import state_transition
        steps = 0
        current_cost = 0
        current_tick_time = 0
        act_num = 1
        ...
        while val != 'success' and val != 'failure':
            ...
            # print("val, obj, cost, tick_time",val, obj, cost, tick_time)
            act_num += 1
            if verbose:
                print(f"Action: {act_num}  {obj.__str__().ljust(35)}cost: {cost}")
            record_act.append(obj.__str__())
...
def collect_conditions(node):
    from btgym.algos.bt_planning.behaviour_tree import Leaf
    conditions = set()
