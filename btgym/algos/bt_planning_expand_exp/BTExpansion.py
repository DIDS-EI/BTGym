...
import heapq
import re
from btgym.algos.bt_planning.behaviour_tree import Leaf, ControlBT
from btgym.algos.bt_planning.Action import Action, state_transition
...
class BTExpansion(BTPAlgo):
    def __init__(self,exp_record=True, max_expanded_num=None,theory_priority_act_ls=[], **kwargs):
        super().__init__(bt_merge=False,max_expanded_num=max_expanded_num,**kwargs)
    ...
    def run_algorithm_selTree(self, start, goal, actions, merge_time=99999999):
        ...
        if goal <= start:
            ...
            # experience new
            if self.exp_record:
                self.expanded_act_ls_ls.append([])
                self.expanded_percentages_ls.append(0)
                ...
                if c <= start:
                    ...
                    if self.exp_record:
                        ...
                        self.expanded_percentages_ls.append(
                            calculate_priority_percentage(self.expanded_act_ls,
                                                          self.theory_priority_act_ls))
                    return bt, min_cost,self.time_limit_exceeded
